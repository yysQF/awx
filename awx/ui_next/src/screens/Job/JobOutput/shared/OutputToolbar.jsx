  ...
  return (
    <Wrapper>
        ...
        <>
          ...
          {failureCount > 0 && (
            <BadgeGroup aria-label={i18n._(t`Failed Host Count`)}>
              <div>{i18n._(t`失败`)}</div>
              <Tooltip content={i18n._(t`失败主机数`)}>
      ...
      <BadgeGroup aria-label={i18n._(t`Elapsed Time`)}>
        <div>{i18n._(t`用时`)}</div>
        <Tooltip content={i18n._(t`作业运行的时间`)}>
      ...
      {job.related?.stdout && (
        <Tooltip content={i18n._(t`下载输出`)}>
          <a href={`${job.related.stdout}?format=txt_download`}>
            <Button variant="plain" aria-label={i18n._(t`下载输出`)}>
              <DownloadIcon />
            </Button>
          </a>
        </Tooltip>
      ...
      {job.summary_fields.user_capabilities.delete && (
        <Tooltip content={i18n._(t`删除作业`)}>
          <DeleteButton
            name={job.name}
            modalTitle={i18n._(t`删除作业`)}
            onConfirm={onDelete}
            variant="plain"
          >
            <TrashAltIcon />
