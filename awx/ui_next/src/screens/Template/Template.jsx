...
function Template({ i18n, setBreadcrumb }) {
  ...
  const tabsArray = [
    ...
    { name: i18n._(t`详情`), link: `${match.url}/details` },
    { name: i18n._(t`使用权`), link: `${match.url}/access` },
  ];
  ...
  tabsArray.push(
    ...
    {
      name: canAddAndEditSurvey ? i18n._(t`问卷调查`) : i18n._(t`View Survey`),
        ...
        <Card>
          <ContentError error={contentError}>
            {contentError.response?.status === 404 && (
              <span>
                {i18n._(t`模板未找到`)}{' '}
                <Link to="/templates">{i18n._(t`查看所有模板`)}</Link>
              </span>
            )}
          </ContentError>
        </Card>
