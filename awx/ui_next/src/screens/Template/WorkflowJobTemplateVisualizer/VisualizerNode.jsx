...
function VisualizerNode({
    ...
    if (addingLink) {
      updateHelpText(
        node.isInvalidLinkTarget
          ? i18n._(
              t`链接目标无效，无法链接到子节点或祖先节点，不支持图形循环.`
            )
          : i18n._(t`单击以创建指向此节点的新链接.`)
      );
      onMouseOver(node);
    }
  };
  ...
  const viewDetailsAction = (
    <WorkflowActionTooltipItem
      ...
      onMouseEnter={() => updateHelpText(i18n._(t`查看节点详情`))}
  ...
  const tooltipActions = readOnly
    ...
    : [
        <WorkflowActionTooltipItem
          ...
          onMouseEnter={() => updateHelpText(i18n._(t`新建节点`))}
          onMouseLeave={() => updateHelpText(null)}
        >
          <PlusIcon />
        </WorkflowActionTooltipItem>,
        ...
        <WorkflowActionTooltipItem
          ...
          onMouseEnter={() => updateHelpText(i18n._(t`编辑节点`))}
          onMouseLeave={() => updateHelpText(null)}
        >
          <PencilAltIcon />
        </WorkflowActionTooltipItem>,
        <WorkflowActionTooltipItem
          ...
          onMouseEnter={() =>
            updateHelpText(i18n._(t`链接到可用节点`))
          }
          onMouseLeave={() => updateHelpText(null)}
        >
          <LinkIcon />
        ...
        <WorkflowActionTooltipItem
          ...
          onMouseEnter={() => updateHelpText(i18n._(t`删除节点`))}
          onMouseLeave={() => updateHelpText(null)}
        >
          <TrashAltIcon />
        </WorkflowActionTooltipItem>,
